...
offset.err  <- segData[TrialNo %in% c(1:5) & ExpNo == "Strobe" & Collection == "Old"]
offset.mean <- offset.err[, .(headingErr = mean(headingErr, na.rm = T)), by = c("SubjectNo", "ExpNo", "DisplayMode", "Familiarity", "seg.z")]

write.csv(offset.mean, "Experiments/Exp2/Analysis/Clustering/Strobe/OldDataset/segData_trialMean.csv")