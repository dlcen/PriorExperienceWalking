...
# Load the data
load('Experiments/Exp3/Data/meanErr.RData')
load('Experiments/Exp3/Data/segData.RData')

segData         <- data.table(segData_clean)
segData.aligned <- data.table(segData_aligned)

offset.segData  			<- segData[TrialNo %in% c(1:5), ]
offset.segData$DisplayMode 	<- droplevels(offset.segData$DisplayMode)
offset.segData[Familiarity == "Unfamiliar"]$TargetPosition <- "Unknown"
offset.segData$Condition      <- paste(offset.segData$Familiarity, "Target", offset.segData$TargetPosition, sep = " ")

offset.segData.aligned  		<- segData.aligned[TrialNo %in% c(1:5), ]
offset.segData.aligned$DisplayMode 	<- droplevels(offset.segData.aligned$DisplayMode)
offset.segData.aligned[Familiarity == "Unfamiliar"]$TargetPosition <- "Unknown"
offset.segData.aligned$Condition      <- paste(offset.segData.aligned$Familiarity, "Target", offset.segData.aligned$TargetPosition, sep = " ")

offset.segMean 			<- offset.segData[, .(x = mean(x, na.rm = T), headingErr = mean(headingErr, na.rm = T)), by = c("SubjectNo", "Familiarity", "TargetPosition", "Condition", "seg.z")]
offset.segMean.aligned 	      <- offset.segData.aligned[, .(x = mean(x, na.rm = T), headingErr = mean(headingErr, na.rm = T)), by = c("SubjectNo", "Familiarity", "TargetPosition", "Condition", "seg.z")]
...
ggplot2::ggsave("Experiments/Exp3/Figures/Figure_1.png", width = 24, height = 12, units = "cm")
