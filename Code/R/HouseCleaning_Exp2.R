...
exp1.Dat_clean          <- Dat_clean
exp1.Dat_raw_clean      <- Dat_raw_clean
exp1.alignedDat_raw     <- alignedDat_raw

exp1.trial.meanErr.long <- trial.meanErr.long
exp1.trial.meanErr.wide <- trial.meanErr.wide

exp1.segData_clean      <- segData_clean
exp1.segData_raw_clean  <- segData_raw_clean
exp1.segData_aligned    <- segData_aligned
...
# Read the data
dat <- read.csv("Experiments/Exp2/Data/Data_raw_all.csv", check.names = F)
dat <- data.table(dat)
...
with(trial.meanErr.wide, table(Collection, Familiarity, DisplayMode, ExpNo))
...
weirdoNo <- as.character(trial.meanErr.long$SubjectNo[weirdo]); weirdoNo     # S_S159
weirdoTrial <- as.numeric( trial.meanErr.long$TrialNo[weirdo]); weirdoTrial  # 2
...
exp.conditions <- as.character(unique(trial.meanErr.long$ExpNo))
...
for (e in exp.conditions) {
      for (d in dis.conditions) {
            for (f in fml.conditions) {
                  for (i in 1:10) {
                        thisTrial <- trial.meanErr.long[ExpNo == e & DisplayMode == d & Familiarity == f & TrialNo == i ]
...
# Absolute z-score greater than 3  6 :  L_S178 
# Absolute z-score greater than 3  8 :  L_S178 
# Absolute z-score greater than 3  4 :  L_S20 
# Absolute z-score greater than 3  10 :  L_S140 
# Absolute z-score greater than 3  4 :  S_S173 
# Absolute z-score greater than 3  8 :  S_S154 
...
save(Dat_clean, Dat_raw_clean, alignedDat_raw, file = "Experiments/Exp2/Data/Data_clean.RData")
save(trial.meanErr.long, trial.meanErr.wide, file = "Experiments/Exp2/Data/meanErr.RData")
save(segData_clean, segData_raw_clean, segData_aligned, file = "Experiments/Exp2/Data/segData.RData")
save(endDat, file = "Experiments/Exp2/Data/endData.RData")
save(total.n.trials.new, total.n.trials.old, n.outside.range.trials, n.outside.3SD.trials, pre.subjects.new, pre.subjects.old, post.subjects.new, post.subjects.old, file = "Experiments/Exp2/Data/Subjects.RData")
